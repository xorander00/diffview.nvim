...
local Commit = require("diffview.vcs.adapters.fossil.commit").FossilCommit
local RevType = require("diffview.vcs.rev").RevType
local FossilRev = require('diffview.vcs.adapters.fossil.rev').fossilRev
...
local FossilAdapter = oop.create_class('FossilAdapter', VCSAdapter)

FossilAdapter.Rev = FossilRev
FossilAdapter.config_key = "fossil"
FossilAdapter.bootstrap = {
  done = false,
  ok = false,
  version = {},
...
function FossilAdapter.run_bootstrap()
  local fossil_cmd = config.get_config().fossil_cmd
  local bs = FossilAdapter.bootstrap
  bs.done = true

  local function err(msg)
    if msg then
      bs.err = msg
      logger.error("[FossilAdapter] " .. bs.err)
    end
  end

  if vim.fn.executable(fossil_cmd[1]) ~= 1 then
    return err(("Configured `fossil_cmd` is not executable: '%s'"):format(fossil_cmd[1]))
  end

  local out = utils.system_list(vim.tbl_flatten({ fossil_cmd, "version" }))
...
function FossilAdapter.get_repo_paths(path_args, cpath)
  local paths = {}
...
local function get_toplevel(path)
  local out, code = utils.system_list(vim.tbl_flatten({config.get_config().fossil_cmd, {"root"}}), path)
  if code ~= 0 then
    return nil
  end
...
function FossilAdapter.find_toplevel(top_indicators)
  local toplevel
...
---@param cpath string?
---@return string? err
---@return FossilAdapter
function FossilAdapter.create(toplevel, path_args, cpath)
  local err
  local adapter = FossilAdapter({
    toplevel = toplevel,
    path_args = path_args,
    cpath = cpath,
  })
...
function FossilAdapter:init(opt)
  opt = opt or {}
  FossilAdapter:super().init(self, opt)
...
function FossilAdapter:get_command()
  return config.get_config().fossil_cmd
end

function FossilAdapter:get_show_args(path, rev)
  return utils.vec_join(self:args(), "cat", "--rev", rev:object_name(), "--", path)
end

function FossilAdapter:get_log_args(args)
  return utils.vec_join("log", "--stat", '--rev', args)
end

function FossilAdapter:get_merge_context()
  local ret = {}
  ...
  for _, file in ipairs(data[1].files) do
    for _, extra in ipairs(file.extras) do
      if (extra.key == 'ancestorlinknode' and extra.value ~= FossilAdapter.Rev.NULL_TREE_SHA) then
...
function FossilAdapter:file_history_options(range, paths, argo)
  local cfile = pl:vim_expand("%")
  cfile = pl:readlink(cfile) or cfile
  ...
  if range then
    utils.err("Line ranges are not supported for fossil!")
    return
  end
  ...
  if not ok then
    utils.info({
      ("No fossil history for the target(s) given the current options! Targets: %s")
...
---@class FossilAdapter.PreparedLogOpts
---@field rev_range string
---@field base Rev
---@field path_args string[]
---@field flags string[]

---@class FossilAdapter.FHState
---@field thread thread
---@field path_args string[]
---@field log_options FossilLogOptions
---@field prepared_log_opts FossilAdapter.PreparedLogOpts
...
---@param log_options FossilLogOptions
---@param single_file boolean
---@return FossilAdapter.PreparedLogOpts
function FossilAdapter:prepare_fh_options(log_options, single_file)
  local o = log_options
...
---@param log_opt FossilLogOptions
---@return boolean ok, string description
function FossilAdapter:file_history_dry_run(log_opt)
  local single_file = self:is_single_file(log_opt.path_args)
  local log_options = config.get_log_options(single_file, log_opt, self.config_key) --[[@as FossilLogOptions ]]
  ...
  log_options = utils.tbl_clone(log_options) --[[@as FossilLogOptions ]]
  log_options.limit = 1
  -- TODO
  options = self:prepare_fh_options(log_options, single_file).flags

  local context = "FossilAdapter.file_history_dry_run()"
...
---@param self FossilAdapter
---@param state FossilAdapter.FHState
---@param callback fun(status: JobStatus, data?: table, msg?: string[])
FossilAdapter.incremental_fh_data = async.void(function(self, state, callback)
  local raw = {}
  ...
  local debug_opt = {
    context = "FossilAdapter:incremental_fh_data()",
    func = "s_info",
    no_stdout = true,
  }
...
---@param state FossilAdapter.FHState
function FossilAdapter:parse_fh_data(state)
  local cur = state.cur
  ...
  if cur.merge_hash and cur.numstat[1] and #cur.numstat ~= #cur.namestat then
    ...
    local max_retries = 2
    local context = "FossilAdapter:parse_fh_data()"
  ...
  for i = 1, #cur.numstat - 1 do
    ...
    table.insert(files, FileEntry.with_layout(state.opt.default_layout or Diff2Hor, {
      ...
      commit = state.commit,
      revs = {
        a = cur.left_hash and FossilRev(RevType.COMMIT, cur.left_hash) or FossilRev.new_null_tree(),
        b = state.prepared_log_opts.base or FossilRev(RevType.COMMIT, cur.right_hash),
      }
    }))
  end
...
function FossilAdapter:is_single_file(path_args, lflags)
...
function FossilAdapter:file_history_worker(thread, log_opt, opt, co_state, callback)
  ---@type LogEntry[]
  local entries = {}
  local data = {}
  local data_idx = 1
...
---@param argo ArgObject
function FossilAdapter:diffview_options(argo)
  ...
  local options = {
    show_untracked = true, -- TODO: extract from fossil config
...
function FossilAdapter:rev_to_pretty_string(left, right)
...
function FossilAdapter:head_rev()
  ...
  return FossilRev(RevType.COMMIT, s, true)
end

function FossilAdapter:rev_to_args(left, right)
  assert(
...
---@return boolean
function FossilAdapter:is_rev_arg_range(rev_arg)
...
---@return Rev? left
---@return Rev? right
function FossilAdapter:parse_revs(rev_arg, opt)
  ---@type Rev?
  local left
  ---@type Rev?
  local right
  ...
  if not rev_arg then
    left = head or FossilRev.new_null_tree()
    right = FossilRev(RevType.LOCAL)
  else
    local from, to = rev_arg:match("([^:]*)%:%:?(.*)$")

    if from and from ~= ""  and to and to ~= "" then
      left = FossilRev(RevType.COMMIT, from)
      right = FossilRev(RevType.COMMIT, to)
    elseif from and from ~= "" then
      left = FossilRev(RevType.COMMIT, from)
      right = head
    elseif to and to ~= "" then
      left = FossilRev.new_null_tree()
      right = FossilRev(RevType.COMMIT, to)
    else
      ...
      left = FossilRev(RevType.COMMIT, node[1])
      ...
      right = FossilRev(RevType.COMMIT, node[1])
...
function FossilAdapter:file_restore(path, kind, commit)
  local _, code
  local abs_path = utils.path:join(self.ctx.toplevel, path)

  _, code = self:exec_sync({"cat", "--", path}, self.ctx.toplevel)

  local exists_fossil = code == 0

  local undo

  if not exists_fossil then
...
function FossilAdapter:get_files_args(args)
  return utils.vec_join(self:args(), "status", "--print0", "--unknown", "--no-status", "--template={path}\\n", args)
end

FossilAdapter.tracked_files = async.wrap(function (self, left, right, args, kind, opt, callback)
  ---@type FileEntry[]
  local files = {}
  ---@type FileEntry[]
  local conflicts = {}
  ...
  local debug_opt = {
    context = "FossilAdapter>tracked_files()",
    func = "s_debug",
    debug_level = 1,
    no_stdout = true,
  }
  ...
  then
    out_status =
      vcs_utils.ensure_output(2, { namestat_job, numstat_job }, "FossilAdapter>tracked_files()")
  end
...
FossilAdapter.untracked_files = async.wrap(function(self, left, right, opt, callback)
  Job:new({
    command = self:bin(),
    args = utils.vec_join(
      self:args(),
    ...
    on_exit = function(j)
      utils.handle_job(j, {
        debug_opt = {
          context = "FossilAdapter>untracked_files()",
          func = "s_debug",
          debug_level = 1,
          no_stdout = true,
        },
...
FossilAdapter.show = async.wrap(function(self, path, rev, callback)
  ...
  local job = Job:new({
    ...
    on_exit = async.void(function(j)
      local context = "FossilAdapter.show()"
      utils.handle_job(j, {
        fail_on_empty = true,
        context = context,
...
-- Add reverse lookups
for _, list in pairs(FossilAdapter.flags) do
...
function FossilAdapter:is_binary(path, rev)
  -- TODO
  return false
end

-- TODO: implement completion
function FossilAdapter:rev_candidates(arg_lead, opt)
...
function FossilAdapter:init_completion()
